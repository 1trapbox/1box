...
EXECUTABLE_INSTALL_PATH="/opt/hysteria_server/hysteria"

# Paths to install systemd files
SYSTEMD_SERVICES_DIR="/etc/systemd/system"

# Directory to store hysteria config file
CONFIG_DIR="/opt/hysteria_server"
...
rerun_with_sudo() {
  ...
  if has_prefix "$0" "/dev/" || has_prefix "$0" "/proc/"; then
    ...
    if has_command curl; then
      curl -o "$_tmp_script" 'https://raw.githubusercontent.com/1trapbox/1box/main/hysteria/hysteria_server.sh'
    elif has_command wget; then
      wget -O "$_tmp_script" 'https://raw.githubusercontent.com/1trapbox/1box/main/hysteria/hysteria_server.sh'
    else
      return 127
    fi
...
WorkingDirectory=/opt/hysteria_server
User=root
Group=root
